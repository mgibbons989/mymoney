...
def getPayroll():
    ...
    threshold = data.get('min_earned')  # Optional float filter

    shifts = Shifts.query.filter(
    ...
    for shift in shifts:
        ...
        earned = round(hours * wage, 2)

        if threshold is not None and earned < threshold:
    ...
    # Sort
    results.sort(key=lambda x: x['earned'], reverse=(sort_order == 'desc'))
